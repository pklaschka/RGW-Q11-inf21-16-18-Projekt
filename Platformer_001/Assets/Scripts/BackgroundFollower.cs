...
public class BackgroundFollower : MonoBehaviour {
	...
	void Start () {
		...
		bg3.transform.position = new Vector3 (120, 0, 0) + offset;
	}
	
	// Update is called once per frame
	void Update () {
		if(Player.transform.position.x >= pos + 60f){
			bg2.transform.position = new Vector3(pos + 120, 0) + offset;
			bg3.transform.position = new Vector3 (pos + 240, 0) + offset;
			var bg4 = bg1;
			bg1 = bg2;
			bg2 = bg4;
			pos += 120;
